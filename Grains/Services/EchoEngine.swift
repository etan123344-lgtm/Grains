...
final class EchoEngine {
    ...
    func process(inputL: UnsafeMutablePointer<Float>,
        ...
        // Delay time in samples (both channels share the same length for ping-pong)
        let delayMs = min(max(params.delayTime, 1), EchoEngine.maxDelaySeconds * 1000)
        let delaySamples = Int(delayMs * sampleRate / 1000.0)

        delayL.delayLength = min(delaySamples, delayL.maxDelay)
        delayR.delayLength = min(delaySamples, delayR.maxDelay)
